...
show_help() {
    ...
    echo "  -f --file"
    echo "          specify the Dockerfile name. Default is 'Dockerfile'."
}

DOCKERFILE_NAME="Dockerfile" # 默认Dockerfile名称
...
while [[ "$#" -ge 1 ]]; do
    case $1 in
        ...
        -f|--file)
            ...
            shift
            if [[ "$#" -eq 0 ]]; then
                echo "Dockerfile name shall not be empty."
                echo ""
                show_help
                exit 1
            fi
...
cd ../.. && docker build . --no-cache -f deployment/prod/${DOCKERFILE_NAME} -t ${IMAGE_TAG} \
