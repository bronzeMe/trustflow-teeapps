...
# RUN conda config --add channels  https://mirrors.tuna.tsinghua.edu.cn/anaconda/pkgs/msys2
...
RUN apt update && apt install -f && apt install -y dh-autoreconf
RUN apt update && apt install -y dh-autoreconf && apt purge -y gcc-9 g++-9
